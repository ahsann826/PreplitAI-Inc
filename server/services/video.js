...
async function generateVideoFromScenes(scenesJsonPath, opts = {}) {
  await ensureDirs();
  const projectRoot = path.join(__dirname, '..', '..');
  const py = resolvePython();

  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  const baseName = path.basename(scenesJsonPath, path.extname(scenesJsonPath));
  const outMp4 = path.join(__dirname, '..', 'outputs', 'videos', `${baseName}_${ts}.mp4`);

  const args = [
    '-m', 'ml.cli',
    'dummy.txt', // Positional argument required by argparse (though we'll ignore it)
    '--scenes-json', scenesJsonPath,
    '--tts-provider', opts.ttsProvider || 'edge',
    '--theme', opts.theme || 'dark',
    '--fps', String(opts.fps || 24),
    '--width', String(opts.width || 1280),
...
module.exports = { generateVideoFromTextFile, generateVideoFromScenes };
