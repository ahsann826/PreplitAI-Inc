  ...
  return (
    <section className="py-24 px-6 bg-[#faf9f7] dark:bg-[#111]">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-14">
          <p className="text-[13px] font-semibold uppercase tracking-widest text-[#630000] mb-3">Features</p>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {features.map((f, i) => (
            <div
              key={i}
              className={`rounded-2xl border border-white/10 overflow-hidden ${f.accent} group cursor-pointer`}
            >
              <div className="p-8 pb-0">
                ...
                <h3 className="text-2xl font-bold tracking-tight text-white whitespace-pre-line leading-tight mb-2">
                  {f.headline}
                </h3>
                <p className="text-[14px] text-white/50 mb-6">{f.description}</p>
              </div>
              {f.image && (
                <div className="px-8">
                  <img
                    src={f.image}
                    alt={f.label}
                    className="w-full rounded-t-xl shadow-md border border-white/10 object-cover"
                  />
                </div>
              )}
            </div>
